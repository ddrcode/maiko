...
Maiko is a topology-aware event router with actor lifecycle management for Tokio. It sits between raw Tokio and full actor frameworks:

- **Topic-based routing** — actors subscribe to topics, not to each other. Publishers don't know who's listening. Adding a consumer never changes a producer.
...
Think of it as moving from breadboard to PCB — same components, reliable traces, testable as a whole.
